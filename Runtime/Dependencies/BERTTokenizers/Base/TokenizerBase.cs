...
using Ummi.Runtime.Speech.Tokenizer.Helpers;
using UnityEngine;
...
namespace Ummi.Runtime.Speech.Tokenizer.Base {
  public abstract class TokenizerBase {
    ...
    public List<(long InputIds, long TokenTypeIds, long AttentionMask)> Encode(int sequenceLength,
      params string[] texts) {
    ...
    private IEnumerable<(string Token, int VocabularyIndex)> TokenizeSubwords(string word) {
      if (_vocabularyDict.ContainsKey(word)) return new (string, int)[] { (word, _vocabularyDict[word]) };
